const STORAGE_TOKEN_KEY = "arena-access-token";
const STORAGE_SLUG_KEY = "arena-user-slug";
const STORAGE_GROUPS_KEY = "arena-saved-groups";
const PLUGIN_DATA_KEY = "arena-imported-blocks";
...
function getImportedBlockIds(): string[] {
...
function addImportedBlockId(blockId: string): string[] {
...
// ── Are.na API fetch (runs in plugin sandbox — no CORS issues) ──────────────

async function arenaFetch(path: string, token: string | null): Promise<any> {
  let url = ARENA_BASE + path;

  const headers: { [name: string]: string } = {};

  if (token) {
    // Use both methods: query parameter AND Authorization header
...
// ── Startup: restore token + send imported IDs ──────────────────────────────
...
figma.ui.onmessage = async (msg: {
  ...
  switch (msg.type) {
    ...
    case "arena-fetch": {
      ...
      const useAuth: boolean = msg.useAuth;
      const token: string | null = msg.token || null;
